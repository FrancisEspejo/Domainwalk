...
Audit the public surface of a domain and see what is set up correctly and what is
not. DNS records, DNSSEC, SPF, DKIM, DMARC, TLS certificates, HTTP security
headers, `security.txt` and `robots.txt`, in a single pass and a single report.

```bash
pip install "domainwalk[crypto]"
...
If your system blocks that with `externally-managed-environment`, which happens on
Debian, Ubuntu and recent Fedora, you have two clean options.

**pipx**, recommended for a command line tool. It creates an isolated environment
for you and puts `domainwalk` on your PATH.

```bash
sudo apt install pipx        # or: brew install pipx
pipx install "domainwalk[crypto]"
```

**A virtualenv**, if you prefer to manage it yourself.
...
### Why the `[crypto]` extra

`pip install domainwalk` works too, with `dnspython` and `rich` as the only
dependencies. The extra adds `cryptography`, which is used to read certificates
that OpenSSL rejects. Without it that path falls back to a private CPython API
that works today but carries no stability promise. Both produce identical
results, and a test enforces that, so the extra is about durability rather than
features.
...
### A domain that is in good shape
...
A well configured domain comes back clean, because facts never count as problems.
That is what makes the red mean something when it does show up.

### A domain with work to do
...
The *How to fix* section is the point. "Missing Referrer-Policy" sends you to a
search engine. The literal header value is something you paste into a config.
...
Knowing that a handshake failed is nearly useless. Knowing the certificate expired
4,157 days ago and who issued it is the whole story. Note that `http.redirect` is
still green, because port 80 answered correctly. The failure is on the HTTPS side.

## Comparing two runs

Save a report now, compare against it later. Useful while you are fixing a domain
and want to confirm what actually moved, or when you revisit one you audited
months ago.
...
The comparison works on severity levels, not text, so a certificate renewal that
leaves `tls.expiry` green produces nothing while a DMARC policy dropping from
`reject` to `none` shows up as a regression.

Output is deterministic. Record lists are sorted and hostnames normalized, so the
RRset rotation your resolver performs on every query never appears as a fake
change. `-o` always writes the plain report, even alongside `--diff`, so any saved
report works as a baseline later. The comparison itself goes to `--diff-output`,
or to stdout with `--json`.

## Muting findings you cannot fix

Some findings are real but unfixable in a given setup. You cannot set custom
response headers on GitHub Pages, for example, and a red flag you can never clear
is noise you learn to ignore.
...
their reason attached, so months later you know why they are quiet. Patterns work,
so `hdr.*` covers every header check. Use `--no-config` to ignore configuration
entirely.
...
language independent, so they are what a script should key off.
...
domainwalk is not a vulnerability scanner, a port scanner or a fuzzer. It does not
read code, audit dependencies or hunt CVEs, and it never sends a request a browser
would not send. It checks configuration, meaning things you fix by publishing a
DNS record or adding a line to a server config.
...
Issues and pull requests are welcome. New checks should follow the existing shape,
a stable id, a severity that reflects real impact, and a `fix` that tells the user
exactly what to publish. If a finding cannot be acted on, it is probably `info`.
